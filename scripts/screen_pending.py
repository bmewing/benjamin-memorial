...
The live path screens each photo as it arrives, but a browser that closed too
early, a restart mid-upload, or a spell with no API key all leave gaps. This
finds them and fills them in. Safe to re-run; it only touches items with no
verdict yet.
...
def main() -> None:
    ...
    for page in s3.get_paginator("list_objects_v2").paginate(Bucket=storage.BUCKET):
        for obj in page.get("Contents", []):
            ...
            elif not k.endswith(".json") and obj["Size"]:
                keys.append(k)

    todo = keys if redo else [k for k in keys if k not in reviewed]
    print(f"{len(keys)} items, {len(reviewed)} already screened, {len(todo)} to do\n")

    for k in todo:
        v = moderation.review_key(storage, k)
